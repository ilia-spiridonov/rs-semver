...
pub fn parse_dot_sep_list(s: &str, check_part: fn(&str) -> bool) -> Option<(&str, &str)> {
    let mut r = s;
...
fn test_parse_dot_sep_list() {
    ...
    assert_eq!(None, parse_dot_sep_list("foo.💩.bar", |_| true));
    assert_eq!(Some(("foo.01", "")), parse_dot_sep_list("foo.01", |_| true));
    assert_eq!(Some(("a", "💩.b")), parse_dot_sep_list("a💩.b", |_| true));
    assert_eq!(Some(("-Ab1", "_")), parse_dot_sep_list("-Ab1_", |_| true));
}
